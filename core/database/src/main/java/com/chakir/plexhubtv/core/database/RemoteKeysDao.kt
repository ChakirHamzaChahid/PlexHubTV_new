...
interface RemoteKeysDao {
    ...
    @Query(
        "SELECT * FROM remote_keys WHERE libraryKey = :libraryKey AND filter = :filter AND sortOrder = :sortOrder AND offset <= :offset ORDER BY offset DESC LIMIT 1",
    )
    suspend fun getClosestKey(
        libraryKey: String,
        filter: String,
        sortOrder: String,
        offset: Int,
    ...
    @Query(
        "SELECT * FROM remote_keys WHERE libraryKey = :libraryKey AND filter = :filter AND sortOrder = :sortOrder ORDER BY offset ASC LIMIT 1",
    )
    suspend fun getFirstKey(
        libraryKey: String,
        filter: String,
        sortOrder: String,
    ): RemoteKey?
