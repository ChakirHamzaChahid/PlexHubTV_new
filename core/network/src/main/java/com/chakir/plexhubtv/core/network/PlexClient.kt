...
class PlexClient(
    ...
    suspend fun getMetadataByGuid(guid: String): Response<PlexResponse> {
        return api.getMetadata(buildUrl("/library/metadata?guid=$guid&includeGuids=1&includeMeta=1"))
    }

    suspend fun getCollectionHubs(collectionId: String): Response<GenericPlexResponse> {
        return api.getHubs(buildUrl("/library/collections/$collectionId/hubs?includeGuids=1&includeMeta=1"))
    }
