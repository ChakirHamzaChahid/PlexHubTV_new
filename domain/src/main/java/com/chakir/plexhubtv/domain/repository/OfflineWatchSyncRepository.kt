...
interface OfflineWatchSyncRepository {
    ...
    fun isWatchedByProgress(
        viewOffset: Long,
        duration: Long,
    ): Boolean = if (duration == 0L) false else (viewOffset.toDouble() / duration) >= WATCHED_THRESHOLD
}
