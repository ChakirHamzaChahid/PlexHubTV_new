                ...
                try {
                    when (media.type) {
                        ...
                        MediaType.Season -> {
                            ...
                            val firstUnwatched =
                                episodes.find { !isFinished(it) }
                                    ?: episodes.firstOrNull()

                            if (firstUnwatched != null) {
                                Result.success(firstUnwatched)
                            } else {
                                Result.failure(Exception("No episodes found in season"))
                            }
                        }
                        ...
                        MediaType.Show -> {
                            ...
                            if (firstEp != null) {
                                Result.success(firstEp)
                            } else {
                                Result.failure(Exception("No episodes found"))
                            }
                        }
