 ...
 */
@Singleton
class OfflineWatchSyncRepositoryImpl
    @Inject
    constructor(
        ...
        override fun isWatchedByProgress(
            viewOffset: Long,
            duration: Long,
        ): Boolean {
        ...
        override suspend fun queueMarkWatched(
            serverId: String,
            ratingKey: String,
        ) {
        ...
        override suspend fun queueMarkUnwatched(
            serverId: String,
            ratingKey: String,
        ) {
        ...
        override suspend fun getLocalWatchStatusesBatched(globalKeys: Set<String>): Map<String, Boolean?> {
            ...
            val actions =
                offlineWatchDao.getLatestWatchActionsForKeys(globalKeys.toList())
        ...
        override suspend fun syncPendingItems(): Result<Int> {
            return try {
                ...
                for ((serverId, actions) in actionsByServer) {
                    ...
                    val server =
                        authRepository.getServers().getOrNull()
        ...
        private suspend fun syncAction(
            client: PlexClient,
            action: OfflineWatchProgressEntity,
        ) {
            when (action.actionType) {
                ...
                for ((serverId, seasonMap) in episodesByServerAndSeason) {
                    ...
                    for ((seasonRatingKey, downloadedEpisodeKeys) in seasonMap) {
                        try {
                            ...
                            Timber.w(e, "WatchSync: failed to sync season=$seasonRatingKey on server=$serverId")
                        }
                    }
                }
                ...
                for ((serverId, ratingKeys) in nonEpisodeItems) {
                    ...
                    for (ratingKey in ratingKeys) {
                        try {
                            ...
                            Timber.w(e, "WatchSync: failed to sync item=$ratingKey on server=$serverId")
                        }
                    }
                }
