...
class MediaMapper @Inject constructor() {
    ...
    // --- Network to Domain ---
    fun mapDtoToDomain(dto: MetadataDTO, serverId: String, baseUrl: String, accessToken: String?): MediaItem {
        ...
        return MediaItem(
            ...
            // Parts & Streams
            mediaParts = dto.media?.flatMap { mediaDto ->
                mediaDto.parts?.map { partDto ->
                    com.chakir.plexhubtv.domain.model.MediaPart(
                        id = partDto.id,
                        key = partDto.key,
            ...
            // Cast
            role = dto.roles?.map { roleDto ->
                com.chakir.plexhubtv.domain.model.CastMember(
                    id = roleDto.id,
            ...
            chapters = dto.chapters?.map {
                com.chakir.plexhubtv.domain.model.Chapter(
            ...
            markers = dto.markers?.map {
                com.chakir.plexhubtv.domain.model.Marker(
    ...
    private fun mapStream(dto: com.chakir.plexhubtv.data.model.StreamDTO): com.chakir.plexhubtv.domain.model.MediaStream {
        return when (dto.streamType) {
            1 -> {
                val isHdr = dto.colorSpace == "bt2020" || 
                           dto.colorTransfer == "smpte2084" || 
                           dto.colorTransfer == "arib-std-b67" ||
                           dto.profile?.contains("Main 10", ignoreCase = true) == true
                
                com.chakir.plexhubtv.domain.model.VideoStream(
            ...
            2 -> com.chakir.plexhubtv.domain.model.AudioStream(
                id = dto.id, index = dto.index, language = dto.language, languageCode = dto.languageCode,
                title = dto.title, displayTitle = dto.displayTitle, codec = dto.codec, selected = dto.selected,
                channels = dto.channels
            )
            3 -> com.chakir.plexhubtv.domain.model.SubtitleStream(
                id = dto.id, index = dto.index, language = dto.language, languageCode = dto.languageCode,
                title = dto.title, displayTitle = dto.displayTitle, codec = dto.codec, selected = dto.selected,
                forced = dto.forced, key = dto.key
            )
            else -> com.chakir.plexhubtv.domain.model.UnknownStream(
                dto.id, dto.index, dto.language, dto.languageCode, dto.title, dto.displayTitle, dto.codec, dto.selected
            )
        }
    }
    ...
    fun mapDomainToEntity(item: MediaItem, libraryKey: String): MediaEntity {
        return MediaEntity(
