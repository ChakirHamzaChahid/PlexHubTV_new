...
class IptvRepositoryImpl @Inject constructor(
    ...
    override suspend fun refreshChannels(url: String): Result<Unit> = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
        return@withContext try {
            android.util.Log.d("IPTV", "Fetching M3U from: $url")
            val request = Request.Builder().url(url).build()
            val response = okHttpClient.newCall(request).execute()
            
            if (!response.isSuccessful) {
                android.util.Log.e("IPTV", "Failed to fetch M3U: ${response.code}")
                return@withContext Result.failure(IOException("Failed to download M3U: ${response.code}"))
            }

            val inputStream = response.body?.byteStream()
            if (inputStream != null) {
                android.util.Log.d("IPTV", "Parsing M3U...")
                val parsed = M3uParser.parse(inputStream)
                android.util.Log.d("IPTV", "Parsed ${parsed.size} channels")
                _channels.value = parsed
                Result.success(Unit)
            } else {
                android.util.Log.e("IPTV", "Empty response body")
                Result.failure(IOException("Empty response body"))
            }
        } catch (e: Exception) {
            android.util.Log.e("IPTV", "Exception fetching M3U", e)
            Result.failure(e)
        }
    }
    ...
    override suspend fun getM3uUrl(): String? {
        ...
        val buildConfigUrl = com.chakir.plexhubtv.BuildConfig.IPTV_PLAYLIST_URL
