...
class ResolveEpisodeSourcesUseCase @Inject constructor(
    ...
    suspend operator fun invoke(episode: MediaItem): List<MediaSource> = coroutineScope {
        ...
        val matches = otherServers.map { server ->
            async {
                try {
                    android.util.Log.d("ResolveEpisode", "Resolving episode '${episode.title}' on ${server.name}")
                    ...
                    val directMatch = episodeSearch.getOrNull()?.find { candidate ->
                        // Match by GUID if available
                        val idMatch = (episode.imdbId != null && episode.imdbId == candidate.imdbId) ||
                    ...
                    if (directMatch != null) {
                        android.util.Log.d("ResolveEpisode", "Direct Match found: ${directMatch.title}")
                    ...
                    val showMatch = searchResult.getOrNull()?.find { 
                        it.title.equals(showTitle, ignoreCase = true) && it.type == com.chakir.plexhubtv.domain.model.MediaType.Show 
                    } ?: return@async null
                    ...
                    val seasonMatch = seasonsResponse.body()?.mediaContainer?.metadata?.find { 
                    ...
                    val episodeMatch = episodesResponse.body()?.mediaContainer?.metadata?.find { 
                        (it as? MetadataDTO)?.index == episode.episodeIndex 
                    } as? MetadataDTO
                    
                    if (episodeMatch != null) {
                         android.util.Log.d("ResolveEpisode", "Tree Match found: ${episodeMatch.title}")
                         
                         val episodeItem = mapper.mapDtoToDomain(episodeMatch, server.clientIdentifier, baseUrl, server.accessToken)
                    ...
                    android.util.Log.e("ResolveEpisode", "Error resolving on ${server.name}", e)
                    null
                }
            }
