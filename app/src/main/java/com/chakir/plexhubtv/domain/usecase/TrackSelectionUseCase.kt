...
class TrackSelectionUseCase @Inject constructor() {
            ...
            if (realTracks.isNotEmpty()) {
                ...
                if (profile.autoSelectSubtitle == 1) {
                    if (selectedAudioTrack != null && profile.defaultSubtitleLanguage != null) {
                        ...
                        val prefLang = profile.defaultSubtitleLanguage.lowercase()
    ...
    private fun findBestAudioMatch(tracks: List<AudioTrack>, preferred: AudioTrack): AudioTrack? {
    ...
    private fun findBestSubtitleMatch(tracks: List<SubtitleTrack>, preferred: SubtitleTrack): SubtitleTrack? {
        // Exact match
        tracks.firstOrNull {
            it.id == preferred.id && it.title == preferred.title && it.language == preferred.language
        }?.let { return it }
    ...
    private fun buildPreferredLanguages(profile: UserProfile, isAudio: Boolean): List<String> {
    ...
    private fun filterSubtitlesBySDH(tracks: List<SubtitleTrack>, preference: Int): List<SubtitleTrack> {
    ...
    private fun filterSubtitlesByForced(tracks: List<SubtitleTrack>, preference: Int): List<SubtitleTrack> {
        return when (preference) {
            0, 1 -> {
