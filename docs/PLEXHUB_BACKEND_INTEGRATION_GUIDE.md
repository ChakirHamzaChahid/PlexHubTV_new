...
| TMDB present, IMDB manquant | oui | non | `get_movie_external_ids(tmdb_id)` | 1 |
| IMDB present, TMDB manquant | non | oui | Conserver l'IMDB existant | 0 |
| Les deux manquants | non | non | `search_movie()` + `get_movie_external_ids()` | 2 |

3. Seuil de confiance : match accepte si `confidence >= 0.85`
4. Mise a jour Media : `tmdb_id`, `imdb_id`, `unification_id` (`"imdb://tt123"` ou `"tmdb://456"`), `tmdb_match_confidence`
5. Garantie prefixe `tt` : tous les IDs IMDB sont prefixes avec `"tt"` si absent

**Phase 2 : Series** — meme logique avec `search_tv()` et `get_tv_external_ids()`
